...
class _AliasedPayload(BaseModel):
    ...
    report_file: str = Field(alias="reportFile")
...
def test_write_json_serializes_pydantic_payloads(tmp_path: Path) -> None:
    ...
    written_path = adapter.write_json(
        ...
        payload=_AliasedPayload(reportFile="run-1.json"),
    )

    stored_payload = json.loads(written_path.read_text(encoding="utf-8"))

    assert stored_payload == {"reportFile": "run-1.json"}


def test_write_json_serializes_tuple_payloads_as_json_arrays(tmp_path: Path) -> None:
    """Adapter serializes tuples into ordered JSON arrays."""
    adapter = JsonFileWriterAdapter(base_dir=tmp_path)

    written_path = adapter.write_json(label="tuple payload", payload=(1, {"ok": True}, "done"))

    stored_payload = json.loads(written_path.read_text(encoding="utf-8"))

    assert stored_payload == [1, {"ok": True}, "done"]
