"""Domain services for scenario and overall executive summaries."""
...
def build_scenario_executive_summary(*, run_payload: dict[str, Any], source_report_files: Sequence[str]) -> K6ScenarioExecutiveSummary:
    ...
    failing_thresholds = [f"{threshold.metric_key}:{threshold.expression}" for threshold in threshold_results if threshold.status == "fail"]
    if status == "pass":
        executive_note = f"Scenario {scenario_name} met all evaluated thresholds."
    elif status == "fail":
        executive_note = (
            f"Scenario {scenario_name} requires attention due to failing thresholds: " + ", ".join(failing_thresholds)
            if failing_thresholds
            else f"Scenario {scenario_name} requires attention due to threshold failures."
        )
    else:
        executive_note = f"Scenario {scenario_name} has no threshold evaluation data available yet."
...
def build_overall_executive_summary(*, scenario_summaries: list[K6ScenarioExecutiveSummary]) -> K6OverallExecutiveSummary:
    """Build an executive rollup from scenario summary rows."""
    total_scenarios = len(scenario_summaries)
    passed_scenarios = sum(1 for summary in scenario_summaries if summary.status == "pass")
    failed_scenarios = sum(1 for summary in scenario_summaries if summary.status == "fail")
    unknown_scenarios = sum(1 for summary in scenario_summaries if summary.status == "unknown")
    ...
    if total_scenarios == 0:
        ...
        executive_summary = f"All {total_scenarios} scenarios passed their evaluated thresholds."
    elif status == "fail":
        executive_summary = f"{failed_scenarios} of {total_scenarios} scenarios failed threshold evaluation and require attention."
    else:
        executive_summary = f"{unknown_scenarios} of {total_scenarios} scenarios are missing threshold evaluation data."
    ...
    return K6OverallExecutiveSummary(
        ...
        scenarios_requiring_attention=scenarios_requiring_attention,
...
def _derive_scenario_status(*, threshold_results: list[K6ThresholdSummary]) -> K6Status:
